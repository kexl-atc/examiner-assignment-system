' Examiner Assignment System Startup Script (Hidden Mode)
' Version: v8.0.17
...
WshShell.Popup "Service Started!" & vbCrLf & vbCrLf & "Frontend: http://127.0.0.1:8081" & vbCrLf & "Backend: http://127.0.0.1:8082", 0, "v8.0.17", 64
...
Sub CompileSimpleHttpServer()
    On Error Resume Next
    Dim JavaCPath, JavaFile
    JavaCPath = RootDir & "\java-runtime\bin\javac.exe"
    JavaFile = RootDir & "\SimpleHttpServer.java"
    If FSO.FileExists(JavaCPath) Then
        WshShell.Run JavaCPath & " -encoding UTF-8 " & JavaFile, 0, True
    End If
    On Error GoTo 0
End Sub
